...
abstract class BaseTimeFn extends BaseFn {
  ...
  static final EvalFunc<Optional<Integer>> EMPTY_FUNC = args -> empty();
  ...
  final EvalFunc<Optional<Integer>> initYearMonthAtFirst(
      ...
      Function<HiveIntervalYearMonth, Integer> intervalFunc,
      Function<Date, Integer> dateFunc)
    ...
    switch (time.getPrimitiveCategory()) {
      case INTERVAL_YEAR_MONTH:
        return initIntervalYearMonthValueAtFirst(arguments)
            .andThen(interval -> interval.map(intervalFunc));
      case STRING:
      case CHAR:
      case VARCHAR:
      case DATE:
      ...
      case VOID:
        return initDateValueAtFirst(arguments).andThen(date -> date.map(dateFunc));
      default:
  ...
  final EvalFunc<Optional<Integer>> initTimeAtFirst(
      ObjectInspector[] arguments,
      Function<HiveIntervalDayTime, Integer> intervalFunc,
      Function<Timestamp, Integer> timestampFunc)
      throws UDFArgumentTypeException {
    PrimitiveObjectInspector time = (PrimitiveObjectInspector) arguments[0];
    switch (time.getPrimitiveCategory()) {
      ...
      case VOID:
        return initTimestampValueAtFirst(arguments)
            .andThen(timestamp -> timestamp.map(timestampFunc));
      default:
        throw new UDFArgumentTypeException(
            0, getFuncName() + " does not take " + time.getPrimitiveCategory() + " type");
    }
